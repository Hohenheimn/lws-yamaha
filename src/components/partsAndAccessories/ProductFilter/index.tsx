...
const ProductFilter = (props: PropsType) => {
  ...
  const renderSubCategories = (
    subCategories: ProductFilterType["subCategories"]
  ) => {
    return (
      <div className={`pl-6 flex flex-col gap-3`}>
        {subCategories?.map((subCategory) => (
          <div
            key={subCategory.id}
            onClick={() => addSubCategoryId(subCategory.id)}
            className="flex cursor-pointer gap-3 items-center"
          >
            {subCategoryIds.includes(subCategory.id) ? (
              <MdOutlineCheckBox className="text-[#f1f1f1] w-5 h-5" />
            ) : (
              <MdOutlineCheckBoxOutlineBlank className="text-[#f1f1f1] w-5 h-5" />
            )}
    ...
    <div className="bg-[#323232] rounded-lg px-4 py-6 flex flex-col gap-4 max-w-sm">
      {props.categories.map((category) => (
        <div key={category.id} className="flex flex-col gap-3">
          ...
          >
            <FaSquare
              className={`text-[#868686] w-3 h-3 transition-transform ${
                category.id === props.activeCategoryId && "rotate-180"
              }`}
            />
            <span className="text-[#f1f1f1] text-lg font-semibold">
              {category.name}
            </span>
          </div>
