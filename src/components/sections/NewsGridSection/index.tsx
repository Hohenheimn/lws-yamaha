    ...
    return (
        <div className='flex flex-col gap-10 bg-[#323232] p-5 md:p-10'>
            <div className='flex items-end justify-between border-b border-[#f1f1f1] pb-4'>
                ...
                <div>
                    <h2 className='font-bold text-[#f1f1f1] text-3xl md:text-center'>News</h2>
            ...
            <div className='flex flex-col-reverse lg:flex-row gap-4'>
                <div className='flex flex-col gap-4 h-full'>
                    {props.news?.slice(0, 2).map((news) => (
                        <NewsCardGrid key={news.id} {...news} onClick={() => console.log(news)} />
                    ))}
                </div>
                {!!props.news?.[2] && <NewsCardGrid {...props.news[2]} onClick={() => console.log(props.news?.[2])} direction='column' />}
            </div>
        </div>
    )
}
